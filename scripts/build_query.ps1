# Build RaySpace3D Query using conda on Windows

$ErrorActionPreference = "Stop"

$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$ProjectDir = Split-Path -Parent $ScriptDir
$SourceDir = Join-Path $ProjectDir "query"
$BuildDir = Join-Path $ProjectDir "build\query"
...
if (-not $condaEnvPath -or -not (Test-Path $condaEnvPath)) {
    ...
    Write-Host "  cd query" -ForegroundColor Yellow
    Write-Host "  conda env create -f environment-windows.yml" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "Building RaySpace3D Query" -ForegroundColor Cyan
...
cmake -S $SourceDir -B $BuildDir -DCMAKE_BUILD_TYPE=$BuildType -DOptiX_INSTALL_DIR="$OptiXInstallDir"
...
Write-Host "Run examples: $(Join-Path $BuildDir "bin\$BuildType\rayspace_main.exe")" -ForegroundColor Green
Write-Host ""
